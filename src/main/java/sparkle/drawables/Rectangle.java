...
public final class Rectangle extends BasicShape {
    public Rectangle() {
        super();
    }

    public Rectangle(Vector2 position, Vector2 size, Paint fill) {
        super(position, size, fill);
    }

    public Rectangle(Vector2 position, Vector2 size, Paint stroke, float strokeWidth) {
        super(position, size, stroke, strokeWidth);
    }

    public Rectangle(Vector2 position, Vector2 size, Paint fill, Paint stroke, StrokeType strokeType, float strokeWidth, float rotation) {
    ...
    protected void render(Graphics2D graphics, float x, float y) {
        ...
        if (!fillIsTransparent) {
            ...
            graphics.fillRect(Math.round(x), Math.round(y), Math.round(super.getSize().getX()), Math.round(super.getSize().getY()));
        }
        if (!strokeIsTransparent) {
            graphics.setPaint(super.getStroke().getPaint());
            graphics.setStroke(super.strokeObject);
            graphics.drawRect(Math.round(x), Math.round(y), Math.round(super.getSize().getX()), Math.round(super.getSize().getY()));
        }
    }

    @Override
    protected StrokeType getDefaultStrokeType() {
        return StrokeType.SQUARED;
    }
}